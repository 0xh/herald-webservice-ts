...
export const start = () => {
  ...
  const replServer = repl.start({
    ...
    eval: (cmd: string, context: null, filename: string, callback: Function) => {
      const parts = /^(?:(get|post|put|delete)\s+)?(\S+)(?:\s+([\s\S]+))?$/im.exec(cmd.trim());
      if (!parts) {
      ...
      let composedParams: { [i: string]: any } = {};
      ...
      if (params) {
        if (/^(\S+=\S+)(\s+(\S+=\S+))*$/m.test(params)) {
          ...
          method = 'post';
          const [cardnum, password, gpassword] = params.split(/\s+/g);
      ...
      ((testClient as any)[method](path, composedParams) as AxiosPromise).then(res => {
